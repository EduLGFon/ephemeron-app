...
const _ongoingNotificationId = 0x46435553; // 'FCUS' as hex, arbitrary but stable
...
class FocusTimerController extends Notifier<FocusTimerState> {
  ...
  void setLinkedTask(String? taskId) {
    state = state.copyWith(linkedTaskId: () => taskId, linkedHabitId: () => null);
  }

  void setLinkedHabit(String? habitId) {
    state = state.copyWith(linkedHabitId: () => habitId, linkedTaskId: () => null);
  }
  ...
  Future<void> start() async {
    ...
    state = state.copyWith(
      isRunning: true,
      startedAt: () => now,
      pomodoroPhase: state.elapsed == Duration.zero ? PomodoroPhase.work : state.pomodoroPhase,
    );
    ...
    await scheduler.showOngoingNotification(
      ...
      title: state.mode == FocusMode.pomodoro ? 'Focus (Pomodoro)' : 'Focus session',
      startedAt: now,
    );
  ...
  void _tick() {
    ...
    final elapsed = _accumulatedBeforePause + DateTime.now().difference(startedAt);
    ...
    if (state.mode == FocusMode.pomodoro && elapsed >= state.pomodoroTarget) {
      ...
      final nextPhase =
          state.pomodoroPhase == PomodoroPhase.work ? PomodoroPhase.breakTime : PomodoroPhase.work;
  ...
  Future<String?> stop() async {
    ...
    final finalElapsed = state.isRunning
        ...
        : state.elapsed;
    final sessionStart = _trueSessionStart ?? DateTime.now().subtract(finalElapsed);
    ...
    await ref.read(alarmSchedulerProvider).cancelOngoingNotification(_ongoingNotificationId);
